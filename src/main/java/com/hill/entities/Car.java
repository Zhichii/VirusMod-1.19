...
public class Car implements ModInitializer {

    public static final Logger LOGGER = HillModMain.LOGGER;
    ...
    public static class CarEntity extends Entity {
            ...
            if (this.isInvulnerableTo(source)) {
                ...
                if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
                    if (source.getAttacker() instanceof PlayerEntity)
                        if (!((PlayerEntity)source.getAttacker()).getAbilities().creativeMode)
    ...
    public static class CarItem extends Item {
    ...
    public static class CarEntityModel extends EntityModel<CarEntity> {
    ...
    public static EntityType<CarEntity> CAR = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier("hill", "car"),
            EntityType.Builder.create(CarEntity::new, SpawnGroup.MISC).setDimensions(0.75F, 0.75F).build("car")
    );

    public static final Item car = new CarItem(new Item.Settings().group(ItemGroup.TRANSPORTATION));

    public static class CarEntityRenderer extends EntityRenderer<CarEntity> {


        private static final Identifier TEXTURE = new Identifier("hill", "textures/entity/car.png");
        private final CarEntityModel model;

        public CarEntityRenderer(EntityRendererFactory.Context context) {
            super(context);
            this.model = new CarEntityModel(context.getPart(HillModClient.MODEL_CAR_LAYER));
        }
        ...
        public void render(CarEntity carEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
            matrixStack.push();
            matrixStack.scale(-1.0F, -1.0F, 1.0F);
            this.model.setAngles(carEntity, 0.0F, 0.0F, 0.0F, carEntity.getYaw(), carEntity.getPitch());
            VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(this.model.getLayer(TEXTURE));
            this.model.render(matrixStack, vertexConsumer, i, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
            matrixStack.pop();
            super.render(carEntity, carEntity.getYaw(), carEntity.getPitch(), matrixStack, vertexConsumerProvider, i);
        }
    }
}
